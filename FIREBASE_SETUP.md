...
## 7) (예약) 폭염 데이 푸시 — Firebase Cloud Messaging (무료 Spark 가능)
"오늘 부산 34°C 폭염 데이 — 전 코인 1.5배" 알림을 보내는 길. 서버 코드 없이 **콘솔에서 수동 발송** 가능.
1. Firebase 콘솔 → 프로젝트 설정 → **클라우드 메시징** 탭 → "웹 푸시 인증서" → **키 쌍 생성** → 키를 `config.js`의 `fcmVapidKey`에
2. 키를 받으면 게임에 알림 권한 UI + 토큰 등록 코드를 활성화(개발 작업 1회 필요 — 키 받은 뒤 진행)
3. 발송: 콘솔 → **Messaging** → 새 캠페인 → 대상 = 웹앱 → 제목/내용 입력 → 즉시 발송 (폭염특보 날 아침 1분 루틴)
- 주의: iOS는 16.4+ 에서 "홈 화면에 추가"한 PWA만 푸시 수신 가능. 안드로이드 크롬은 브라우저만으로도 OK.
...
## 정산 운영 (수동 검수 정책) — ★적립금 입금 전 필수 절차
클라이언트 게임 특성상 콘솔 조작(규칙 상한 이내)은 100% 차단 불가 → **수동 검수가 최종 방어선**입니다.
주기: **거리 랭킹 = 주간**(키 형식 `W20260608` = 그 주 월요일), **마라톤 = 월간**(`YYYY-MM`).
1. **주간 거리 랭킹**: Firestore → `leaderboard` → `month == 'W{그 주 월요일 YYYYMMDD}'` 필터 → `best` 내림차순 상위 3명
2. **월간 마라톤 랭킹**: Firestore → `marathon` → `month == 'YYYY-MM'` 필터 → `timeMs` 오름차순 상위 3명

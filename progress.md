...
[build 88] ★단디 풀 리디자인 — 강아지 마스코트 (진우님 선택, 굿즈·스티커 원본)
- 타원+다리2 → 강아지형: ①말린 꼬리(지상=살랑·그늘 안도=신나게 t*16·공중=vy 관성 젖힘) ②쫑긋 귀 2개(앞=스킨색+분홍 귓속, 뒤=진한 톤·흠칫=쫑긋·점프 관성·캡 써도 귀 끝 삐져나옴) ③4족 갤럽 사이클(뒷쌍↔앞쌍 위상+쌍 내 0.45 오프셋, 공중=도약 자세) ④주둥이(밝은 톤)+코+입 위치 조정 ⑤더위 혀 헥헥(기존 입 확대 대체, hp 비례 길이+pant 흔들림).
- 색 시스템 개선: 다리·귀뒷면·꼬리 = mix(스킨, 다크, 0.30) — 하드코딩 제거, 스킨 바꾸면 전신 연동.
- 호환 유지: 장비 6종(러닝화=앞발만·슬리브=뒷다리)·슬라이드 squash·숨쉬기·스텀블 wob·오버히트 틴트·petPop 별빛·petFlinch 식은땀(+귀 쫑긋 추가).
- 검증: 10개 상태 조합(지상/점프/낙하/슬라이드/풀장비/헐떡/오버히트/흠칫/안도/스텀블) draw 예외0·300프레임 무결·1.52ms/frame·콘솔0. 시각 최종 판정은 진우님(스샷툴 타임아웃 지속).

[build 87] ★★도전장 시스템 — 친구 대전(고스트 공유, 서버 0원)
- 내 베스트 런 고스트를 링크 코드로 압축(encodeGhostChallenge): v1!거리!이름!dx~h... — 0.3s 다운샘플·delta·base36, 슬라이드 플래그('s') 보존. 5초 런=119자, 3분 런 추정 ~3KB(URL 해시 안전권).
- 공유: 결과 화면 '공유' 버튼이 자동으로 도전 링크(#vs=코드) 포함 — "내 고스트랑 직접 붙어봐 →". 받는 사람: 링크 열면 '도전장 도착! ○○의 NNNm — 달려서 꺾어!' 배너 + 모든 런에 친구 고스트(핑크, '○○의 기록' 라벨)가 함께 달림 + 추월 시 '○○의 기록 추월!' 보너스.
- 바이럴 메커니즘: 기록 공유=곧 플레이 초대장. 서버·DB 0원(기록이 링크 자체에 들어감).
- 보안: 디코드 sanity — dist≤15000·샘플≤2200·dx≤2000·버전 체크, 변조 코드 거부.
- drawGhost가 g.step(0.3s) 지원·vs=핑크/내 고스트=하늘색 구분. QA: HR.encodeChallenge/decodeChallenge/setVsGhost.
- 검증: 라운드트립(거리·슬라이드·단조성)·vs 렌더·추월 배너·변조 거부 3종·무결성·콘솔0.
- 다음 대형 후보(진우님 선택 대기): ①태양 보스전(세트피스) ②단디 캐릭터 리디자인(풀 애니메이션) ③리플레이 영상 자동 캡처(릴스 직행).

[build 85] 시그니처 모티프 — "이 게임의 노래" (감독 처방)
- MOTIF 7음: 0-4-7-12(상승, 달려나감) → 9-12-14(정점 응답, 9th로 열린 마무리="계속 달린다"). 펜타토닉이라 기존 BGM과 화성 호환.
- 배치: 출발(finishWarmup)=첫 4음 빠르게(매 출발 각인) / 홈 진입(세션 1회, 오디오 언락 후)=옥타브↓ 느긋 / 마라톤 완주=풀 모티프+화음(루트+5도) / 보상 팡파레 sfx도 모티프 음형(0-4-7-12-14)으로 통일.
- BGM 변주: bgmTick 멜로디가 랜덤 펜타 → 모티프 윤곽 순회 75%+장식음 25% — 벽지에서 변주곡으로.
- QA: HR.playMotif('start'|'calm'|'fanfare'). 검증: 5개 호출 경로 예외0·콘솔0. ※실제 음은 진우님 라이브 청취 평가가 최종(별로면 MOTIF 배열만 교체).

[build 84] 배경 입체감 패키지 (진우님: 더 입체적·고품질)
- 대기 베일(drawHaze): 수평선 하늘색으로 far(0.30)/mid(0.14) 레이어를 녹임 — 거리감 핵심. 밴드 양자화 캐시.
- 초원거리 능선(drawFarRidge, parallax 0.07) + 전경 스침 풀(drawForeground, parallax 1.45, 화면 최하단 — 펫보다 앞) → 깊이 켜 4→7겹.
- 성능(감독 P2 동시 해결): drawSky 그라디언트 밴드 1/24 양자화 캐시·지면 그라디언트 캐시. 실측 평균 2.78ms/frame(예산 17%).
- 검증: 장거리 워프 무결성·콘솔0. (스크린샷 툴 타임아웃 지속 — 시각 최종 판정은 진우님)

[브랜드 컬러·타이포 정합 리뷰] (감독 처방 — summertect-brand-guidelines 스킬 기준)
- 합격: 라임 #A7D500=PANTONE 2291C 신호색 역할 일치(CTA·강조·표면라인), grape #3D4A35+지형 변형=기반색, 무채 다크 베이스 — 가이드 비율(무채 70~80/grape 15~20/라임 5~10) 구조 부합. 금지어(역대급/미친/혁신 등) 사용자 노출 문구 0건. 카피 톤('오늘은 SUMMERTECT가 필요한 날' 등) 절제 기조 유지.
- 게임 한정 예외(허용 판정): amber #FFD24D(코인/보상)·blue #74c7ec(물/쿨링)·orange(열) = 게임 기능색 — 가독성 필수라 게임 내 유지. 단 마케팅 자산(카드뉴스 등) 제작 시엔 브랜드 2색+무채만.
- 운영 권고: Black Han Sans는 가이드 외 서체(한글 임팩트용 게임 한정 유지) — 마케팅 자산은 Anton+Pretendard만. 색상 HEX 공식 변환값은 브랜드 운영 파일에서 확정 관리 필요.

[build 83] PWA + 폭염 데이 푸시 준비 (감독 처방③)
- manifest.webmanifest(standalone·portrait·#14180f) + icon.svg(다크+라임 러너+태양, any maskable) + sw.js.
- ★SW 전략 = network-first(온라인이면 항상 서버 최신 → ?v= 캐시버스트와 충돌 0, 빌드 갇힘 사고 원천 차단). 오프라인만 캐시 폴백(ignoreSearch). same-origin만 캐시(폰트/Firebase/날씨 API 불간섭). 등록은 https만(프리뷰/file:// 스킵).
- 설치 유도: beforeinstallprompt 캡처 → 홈 '홈 화면에 설치' 버튼(설치 가능할 때만 노출, appinstalled 시 숨김). iOS 메타(apple-mobile-web-app-*) 추가.
- FCM 푸시: config.fcmVapidKey 자리 + FIREBASE_SETUP.md 7) 가이드(웹 푸시 인증서 키 생성→콘솔 Messaging 캠페인 수동 발송, 서버 불필요). 키 수령 후 권한 UI+토큰 코드 활성화(개발 1회).
- 검증: manifest/icon/sw 전부 200·manifest 링크·설치버튼 기본 hidden·SW http 미등록(의도)·게임 무결성·콘솔0.

[build 82] 현수막 서있기 통과 버그 + 감독 사운드·비주얼 폴리시
- ①현수막 '서서 통과' 버그(유저 질문으로 발견): 판정 상단 0.8r → 1.02r(실제 몸 타원 상단). 0.8r이면 서있는 키(54px)와 틈(52px) 차이 2px라 경사·프레임 타이밍에 뒤집힘. 검증: 서있기=사망(경사 5곳 강건)·낮은점프/단일점프 꼭대기=사망·높은 더블점프(280px)=천 위 통과(의도·고급기술)·슬라이드=통과+보너스.
- ②오버히트 심장박동: sfx('heart') 저음 lub-dub, 체온 높을수록 빨라짐(0.95→0.62s 주기). ③보상 팡파레: sfx('fanfare') 도미솔도 — 스트릭·쿠폰에 적용. ④태양 god ray 밴딩: 끝을 글로우 안(2.8r)으로+방사 그라디언트 페이드(캐시) — 줄무늬 현상 제거.
- 감독 잔여 숙제: 시그니처 멜로디(다음 큰 작업 — 모티프 시안 만들어 진우님 청취 확정 방식), 펫 이름(진우님 보류 결정 vs 감독 재권고), 브랜드 컬러 정합 리뷰(별도).
